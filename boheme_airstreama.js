(function () {
  ...
  waitForScene(() => {
    ...
    function resetP(i, randomZ) {
      ...
      pos[i*3+2] = randomZ ? (Math.random() * (Z_FRONT - Z_BACK) + Z_BACK) : (Z_FRONT + Math.random() * 3);
    ...
    function animateStream() {
      ...
      for(let i=0;i<count;i++){
        ...
        let vx=0,vy=Math.sin(aoa)*speed*0.4,vz=-Math.cos(aoa)*speed;
        const r2d=Math.sqrt(lx*lx+ly*ly);
        if(r2d>0.01&&Math.abs(lz)<L_FUSE*0.55){
          if(r2d<R_FUSE*3.0){
            const f=(R_FUSE*R_FUSE)/(r2d*r2d);
            vx+=(lx/r2d)*speed*f*0.6;
            vy+=(ly/r2d)*speed*f*0.6;
            vz-=speed*f*0.2;
          }
          if(r2d<R_FUSE*1.08&&Math.abs(lz)<L_FUSE*0.5){
            const push=R_FUSE*1.2-r2d;
            vx+=(lx/r2d)*push*12;
            vy+=(ly/r2d)*push*12;
          }
        }
        ...
        if(colorBySpeed){
          ...
          const t=Math.min(Math.max((spd/speed-0.8)*3,0),1);
          col[i*3]=0.2+t*0.8;
          col[i*3+1]=0.5*(1-t)+0.2*t;
          col[i*3+2]=1.0-t*0.7;
        } else {
          col[i*3]=0.35; col[i*3+1]=0.65; col[i*3+2]=1.0;
        }
        if(z<Z_BACK-2||z>Z_FRONT+2||Math.abs(x)>SPREAD_X+2||Math.abs(y)>SPREAD_Y+2) resetP(i,false);
      }
    ...
    const licFrag = `
      ...
      vec2 joukowsky(vec2 z){return z+vec2(z.x/(z.x*z.x+z.y*z.y),-z.y/(z.x*z.x+z.y*z.y));}

      vec2 flowVel(vec2 p){
        float R2=uRadius*uRadius;
        float r2=dot(p,p);
        if(r2<R2*1.01) return vec2(0.0);
        float ca=cos(uAoA), sa=sin(uAoA);
        vec2 Uinf=vec2(-sa,-ca);
        vec2 rh=p/sqrt(r2);
        float fac=R2/r2;
        vec2 v=Uinf-Uinf*fac+2.0*dot(Uinf,rh)*rh*fac;
        float circ=-2.0*3.14159*uRadius*sa*1.2;
        float r=sqrt(r2);
      ...
      void main(){
        ...
        for(int i=0;i<48;i++){
          ...
          if(s<0.0005) break;
          fwd+=v/s*dt;
          vec2 tc=fwd/${(SZ*2.0).toFixed(1)}+0.5;
          tc+=v/s*phase*0.015;
          float n=texture2D(uNoise,fract(tc*4.0)).r;
          float w=1.0-float(i)/48.0;
          w*=w;
          accum+=n*w; weight+=w;
        }
        vec2 bwd=p;
        for(int i=0;i<48;i++){
          ...
          if(s<0.0005) break;
          bwd-=v/s*dt;
          vec2 tc=bwd/${(SZ*2.0).toFixed(1)}+0.5;
          tc+=v/s*phase*0.015;
          float n=texture2D(uNoise,fract(tc*4.0)).r;
          float w=1.0-float(i)/48.0;
          w*=w;
        ...
        lic=smoothstep(0.15,0.85,lic);

        vec2 v=flowVel(p);
        float spd=length(v);
        float sf=clamp(spd*0.45,0.0,1.8);

        if(r2<R2*1.01){gl_FragColor=vec4(0.04,0.04,0.10,0.92);return;}

        vec3 c1=vec3(0.05,0.10,0.30);
        vec3 c2=vec3(0.35,0.55,0.85);
        vec3 c3=vec3(0.85,0.92,1.0);
        vec3 c4=vec3(1.0,0.45,0.15);
        vec3 col;
        if(sf<0.6) col=mix(c1,c2,sf/0.6);
        else if(sf<1.2) col=mix(c2,c3,(sf-0.6)/0.6);
        else col=mix(c3,c4,(sf-1.2)/0.6);

        col*=(lic*0.7+0.3);
        float vign=1.0-smoothstep(0.35,0.52,length(vUv-0.5));
        float alpha=mix(0.55,0.85,vign);
    ...
    function createSchlieren() {
      ...
      schlierenUniforms = {
        uNoise:{value:noiseTex},uTime:{value:0},
        uRadius:{value:R_FUSE},uAoA:{value:aoaDeg*Math.PI/180},uSpeed:{value:speed}
      };
      const mat = new THREE.ShaderMaterial({
        vertexShader:licVert, fragmentShader:licFrag,
        uniforms:schlierenUniforms, transparent:true,
    ...
    function updateSliceOrientation() {
      ...
      if (sliceMode==='XY') { schlierenMesh.rotation.set(0,0,0); schlierenMesh.position.set(0,0,0); }
      else { schlierenMesh.rotation.set(-Math.PI/2,0,0); schlierenMesh.position.set(0,0,0); }
    }
    ...
    document.getElementById('asSchlieren').addEventListener('click', () => {
      ...
      if (schlierenActive) { btn.textContent='Schlieren ON'; btn.style.background='#0f3460'; if(!schlierenMesh) createSchlieren(); else schlierenMesh.visible=true; }
      else { btn.textContent='Schlieren OFF'; btn.style.background='#2a2a4a'; if(schlierenMesh) schlierenMesh.visible=false; }
    });
    ...
    document.getElementById('asSlice').addEventListener('click', () => {
      ...
      document.getElementById('asSlice').textContent='Slice: '+sliceMode;
      updateSliceOrientation();
    });
    ...
    console.log('boheme_airstream.js v3 loaded');
  });
})();
